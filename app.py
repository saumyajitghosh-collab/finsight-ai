...
def page_qps():
    ...
    c += '<script>'
    c += """function drawBloch(strategies, probabilities){var c=document.getElementById('blochSphere');if(!c)return;var ctx=c.getContext('2d');var cx=160,cy=160,r=120;ctx.clearRect(0,0,320,320);ctx.strokeStyle='rgba(255,255,255,0.15)';ctx.lineWidth=1;ctx.beginPath();ctx.arc(cx,cy,r,0,2*Math.PI);ctx.stroke();ctx.beginPath();ctx.ellipse(cx,cy,r,r*0.3,0,0,2*Math.PI);ctx.stroke();ctx.beginPath();ctx.ellipse(cx,cy,r*0.3,r,0,0,2*Math.PI);ctx.stroke();ctx.strokeStyle='rgba(255,255,255,0.3)';ctx.beginPath();ctx.moveTo(cx-r-10,cy);ctx.lineTo(cx+r+10,cy);ctx.moveTo(cx,cy-r-10);ctx.lineTo(cx,cy+r+10);ctx.stroke();ctx.fillStyle='rgba(255,255,255,0.5)';ctx.font='10px sans-serif';ctx.fillText('|0>',cx+5,cy-r-5);ctx.fillText('|1>',cx+5,cy+r+12);ctx.fillText('x',cx+r+12,cy+3);var colors=['#7c4dff','#ff5252','#00e676','#ffab00'];var labels=[];if(!strategies){strategies=['Expansion','Aggressive Acquisition','Hold','Conservative Growth'];probabilities=[0.3,0.4,0.2,0.1]}for(var i=0;i<strategies.length&&i<4;i++){var angle=i*Math.PI*0.5+0.3;var prob=probabilities[i]||0.25;var dist=r*(0.3+prob*0.7);var px=cx+dist*Math.cos(angle);var py=cy-dist*Math.sin(angle);ctx.fillStyle=colors[i];ctx.beginPath();ctx.arc(px,py,8,0,2*Math.PI);ctx.fill();ctx.strokeStyle=colors[i];ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(px,py);ctx.stroke();labels.push({name:strategies[i],prob:(prob*100).toFixed(1)+'%',color:colors[i]})}var labelHtml='';labels.forEach(function(l){labelHtml+='<span style="color:'+l.color+'">&#9679;</span> '+l.name+': '+l.prob+' &nbsp; '});var bl=document.getElementById('blochLabels');if(bl)bl.innerHTML=labelHtml}setTimeout(function(){drawBloch()},500)"""
    c += '</script>'
